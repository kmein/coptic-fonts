...
Digital revivals of Coptic types from printed books, traced from registered and averaged
page scans rather than from any font file. One pipeline in `src/`, one directory per face.

| face | source | status |
|---|---|---|
| [`faces/layton/`](faces/layton/README.md) — **Layton Coptic** | Bentley Layton, *A Coptic Grammar* (Harrassowitz 2000); *Zostrien*, BCNH 24 (Peeters 2000) | v1.3 |
...
one subdirectory per scanned book, and are not tracked.
...
(register and average each letter's instances) → `spacing.py`, `bars.py`,
